...
function HomeScreen() {
  ...
  const getDaysUntil = (dateString: string): string => {
    ...
    // The date from the input is already in YYYY-MM-DD, which is interpreted as UTC midnight.
    // To avoid timezone issues, we construct the date explicitly.
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Welcome Banner */}
          <div className="relative p-8 rounded-2xl text-white bg-primary-700/90 dark:bg-primary-800/90 backdrop-blur-lg border border-white/20 dark:border-primary-700/50 overflow-hidden shadow-lg">
              <div className="relative z-10">
                  <h1 className="text-3xl font-bold">Welcome back!</h1>
                  <p className="mt-2 text-primary-100 max-w-sm">Ready to learn today?</p>
                  <Button onClick={() => navigate('/create-exam')} className="mt-6 !bg-white/20 !text-white hover:!bg-white/30 backdrop-blur-sm border border-white/30">
          ...
          <Card>
            <div className="p-0">
              <div className="flex items-start justify-between">
                  <div>
                      <h2 className="text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-white"><ChartBarSquareIcon className="w-6 h-6 text-slate-500"/> Performance Overview</h2>
              ...
              <div className="mt-6 flex justify-between items-center">
                  <div>
                      <p className="font-semibold text-slate-900 dark:text-white">You have solved {todayStats.questionsSolved} questions today</p>
                      <p className="text-sm text-slate-500 dark:text-slate-400">{todayStats.percentage}% Correct answers</p>
                  </div>
                  <div className="flex items-center gap-4 text-sm">
                      <div className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300"><div className="w-3 h-3 rounded-full bg-green-500"/> Correct</div>
                      <div className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300"><div className="w-3 h-3 rounded-full bg-primary-800 dark:bg-primary-900"/> Incorrect</div>
                  </div>
              </div>
            </div>
            <div className="pt-6">
        ...
        <div className="space-y-6">
          <Card className="!p-0 text-white !bg-transparent !border-0 !shadow-none">
              <div className="p-6 bg-slate-800/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-lg border border-slate-700">
          ...
          <Card>
              <h2 className="text-lg font-bold flex items-center gap-2 text-slate-900 dark:text-white"><FireIcon className="w-5 h-5 text-orange-500"/> Activity Streak</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 mb-4">Your activity in the last 3 months.</p>
              <ActivityHeatmap />
              <div className="flex justify-between mt-4 text-sm text-slate-700 dark:text-slate-300">
          ...
          <Card>
            <div className="flex justify-between items-center">
                <h2 className="text-lg font-bold flex items-center gap-2 text-slate-900 dark:text-white"><CalendarIcon className="w-5 h-5 text-slate-500"/> Upcoming Exams</h2>
                <button onClick={() => setIsAddExamModalOpen(true)} className="p-1.5 rounded-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
            ...
            {upcomingExamsToDisplay.length > 0 ? (
                <ul className="space-y-3 mt-4">
                    {upcomingExamsToDisplay.map(exam => (
                        <li key={exam.id} className="flex items-center gap-3 group">
                            <div className="flex-grow">
                                <p className="font-semibold text-slate-800 dark:text-slate-100">{exam.title}</p>
                                <p className="text-sm text-slate-500 dark:text-slate-400">{new Date(exam.date).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</p>
                            </div>
                            <div className="text-sm font-bold text-primary-600 dark:text-primary-400">{getDaysUntil(exam.date)}</div>
                            <button onClick={() => handleDeleteUpcomingExam(exam.id)} className="opacity-0 group-hover:opacity-100 transition-opacity p-1 text-slate-400 hover:text-red-500">
          ...
          <Card>
               <div className="flex justify-between items-center mb-4">
                  <h2 className="text-lg font-bold flex items-center gap-2 text-slate-900 dark:text-white"><ClockIcon className="w-5 h-5 text-slate-500"/> Recent Activity</h2>
                  <button onClick={() => navigate('/history')} className="text-sm font-semibold text-primary-600 dark:text-primary-400 hover:underline">See All</button>
              </div>
                  ...
                  <ul className="space-y-4">
                      {recentActivity.map(item => (
                          <li key={item.examId} className="flex items-center gap-3">
                              ...
                              <div className="flex-grow">
                                  <p className="font-semibold text-slate-900 dark:text-white truncate">{item.exam!.title}</p>
